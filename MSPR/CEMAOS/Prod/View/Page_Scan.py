...
def Scan_page(self):
    ...
    self.BScan = ttk.Button(self.FScan, text="Lancer le scan du réseau", command=lambda:{Scaneur(self)})
...
def Scaneur(self):
    self.BScan.configure(state=tk.DISABLED)
    col = 0
    ro = 1
    for cle, valeur in scan_network().items():
        ...
        if ro != 6:
            ...
            LMachine.grid(column=col, row=ro, sticky=tk.W, padx=[50,0])
            ro+=1
    self.BScan.configure(state=tk.NORMAL)